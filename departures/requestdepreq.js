    ...
    function processRequest(e) {
        if (Http.readyState == 4) {
            ...
            if (data.trainServices != null) {
                ...
                delquan = 0
                document.getElementById("stationname").innerHTML = "Departures from " + data.locationName + ":";
                ...
                for (var i = 0; i < tablesize + 1 ; i++) {
                    ...
                    if (i > 0) {
                        if (data.trainServices[serviceno].delayReason != null) {
                            ...
                            schedtime = data.trainServices[serviceno].std
                            estitime = data.trainServices[serviceno].etd
                            ...
                            destination = data.trainServices[serviceno].destination[0].locationName
                            if (data.trainServices[serviceno].destination.length > 1) {
                                for (var j = 1; j < data.trainServices[serviceno].destination.length; j++) {
                                    destination += " & " + data.trainServices[serviceno].destination[j].locationName 
                                }
                            }
                            if (data.trainServices[serviceno].destination[0].via != null) {
                                destination += " " + data.trainServices[serviceno].destination[0].via
                            }
                            var tableinfo = {
                                0 : schedtime,
                                1 : destination,
                ...
                document.getElementById("stationname").innerHTML = "There are currently no services running from " + data.locationName + ". Please check the national rail website for more information.";
